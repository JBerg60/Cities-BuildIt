...
namespace BuildIt.Geo
{
    ...
    {
        const double earth = 6378.137f;  //radius of the earth in kilometer
        const double pi = Math.PI;
        const double meter1 = 1 / ((2 * pi / 360) * earth) / 1000;
        ...
        public static double Distance(double latitude, double longitude, double otherLatitude, double otherLongitude)
        {
            ...
            return earth * 1000 * (2.0 * Math.Atan2(Math.Sqrt(d3), Math.Sqrt(1.0 - d3)));
        }
    }
}